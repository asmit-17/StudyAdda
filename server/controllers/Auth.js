...
exports.signup = async (req, res) => {
  try {
    ...
    if (existingUser) {
      return res.status(422).json({
        success: false,
        error: "User already exists. Please sign in to continue.",
      });
    }
...
exports.sendotp = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, error: error.message,mess:"Email fail" })
  }
}
